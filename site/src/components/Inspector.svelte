<script lang="ts">
  ...
  let totalSize = $derived(info?.reduce((s: number, p: any) => s + p.size, 0) ?? 0);
  let formatSummary = $derived(() => {
    if (!info) return '';
    const m: Record<string, number> = {};
    info.forEach((p: any) => { m[p.format] = (m[p.format] || 0) + 1; });
...
<div>
  ...
  <div class="mx-auto max-w-3xl flex items-center gap-2.5 px-6 py-3">
    <div class="flex-1 relative flex items-center min-w-0">
      ...
      <input id="fi" type="file" accept=".mcz" onchange={onFile} hidden>
    </div>
    <button
      onclick={() => inspect(url)}
      ...
      <div class="border border-border rounded-[10px] overflow-hidden">
        <table class="w-full border-collapse text-xs">
          ...
          <tbody>
            {#each info as p, i}
              <tr>
                <td class="px-4 py-2 font-mono text-fg {i < info.length - 1 ? 'border-b border-border' : ''}">{i}</td>
                <td class="px-4 py-2 font-mono text-fg3 {i < info.length - 1 ? 'border-b border-border' : ''}">{p.width}×{p.height}</td>
                <td class="px-4 py-2 font-mono text-fg3 {i < info.length - 1 ? 'border-b border-border' : ''}">{p.format}</td>
                <td class="px-4 py-2 font-mono text-fg3 text-right {i < info.length - 1 ? 'border-b border-border' : ''}">{formatSize(p.size)}</td>
                <td class="px-4 py-2 font-mono text-fg3 text-right max-[480px]:hidden {i < info.length - 1 ? 'border-b border-border' : ''}">{p.offset}</td>
              </tr>
            {/each}
          </tbody>
        </table>
